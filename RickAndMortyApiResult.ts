...
const rickAndMortyApiResult: RickAndMortyApiResult = {
...
const characters = rickAndMortyApiResult.results;

let humansAlive = getAllAliveHumans();
console.log(humansAlive);

let names: string[] = getAllNames();
console.log(names);

let something: { name: string, origin: string } [] = getAllNamesWithOrigin();
console.log(something);


function getAllAliveHumans(): Character[] {
    return characters.filter(result => result.species === "Human" && result.status === "Alive");
}

function getAllNames(): string[] {
    return characters.map(result => result.name);
}

function getAllNamesWithOrigin(): { name: string, origin: string }[] {
    return characters.map(result => {
        return {name: result.name, origin: result.origin.name};
    });
}